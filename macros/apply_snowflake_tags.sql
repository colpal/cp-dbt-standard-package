...
{% macro tag_models_on_run_end() %}
    ...
    {% for node_id in graph.nodes %}
        ...
        {% if node.resource_type == 'model' %}
            ...
            {% if node.config is defined and node.config is not none and node.config.get('snowflake_tags') is defined %}
            ...
            {% if node.columns is defined %}
                {% for column_name, column in node.columns.items() %}
                    {% if column.meta is defined and column.meta is not none and column.meta.get('snowflake_tags') is defined %}
